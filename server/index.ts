    ...
    async function requestLatestVersion() {
        ...
        */
        ...
        try {
            ...
            if(data.isOutdated) {
                alt.logWarning(`[ASCENDED-API]: Your plugin: ${data.repository} is outdated. Latest Commit: ${data.latestCommit} | Version (${data.release}) | ${data.releasedAt}`)
            } else {
                alt.logWarning(`[ASCENDED-API]: Your plugin: ${data.repository} is up to date. Latest Commit: ${data.latestCommit} | Version (${data.release}) | ${data.releasedAt}`)
            }
        } catch (error) {
                    if (error.response) {
                alt.logWarning(`[ASCENDED-Versioncheck-API] => No Response from Ascended API Server... Status: ${error.response.status}`);
            } else {
                alt.logWarning(`[ASCENDED-Versioncheck-API] => No Response from Ascended API Server... ${error.message}`);
            }
        }
        return null;
    }
